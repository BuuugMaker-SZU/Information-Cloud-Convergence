...
Page({
  ...
  onLoad: function (options) {
    // 获取初始数据并加载
    console.log('打印to'+options.to);
    let friend = JSON.parse(options.to);
    friend.avatar='http://121.199.38.35:5000/get_avatar?openid=' + friend.id;
  ...
  sendOrderMessage(e) {
    ...
    order.confirmInvitation = false;
    //GoEasyIM自定义消息,实现活动链接发送
  ...
  JudegeInvitation(e) {
    let that=this;
    if (e.currentTarget.dataset.message.payload.openid != app.globalData.userInfo.openid) {
      // 发送 POST 请求，将该用户加入小组
      wx.request({
        url: 'http://121.199.38.35:5000/signup',
        method: 'POST',
        data: {
            groupid:e.currentTarget.dataset.message.payload.groupid,
        ...
        success: function (res) {
          ...
          if(code===200){
          console.log(e);
          let order = e.currentTarget.dataset.message.payload;
    
          // 将 confirmInvitation 的值设置为 true
          order.confirmInvitation = true;
  ...
  showOrderMessageList() {
    ...
    wx.request({
      url: 'http://121.199.38.35:5000/getResGroup',
      method: 'GET',
      data: {
        openid: openid
      },
      success: (res) => {
        this.setData({
          ['orderList.orders']: res.data.results,
